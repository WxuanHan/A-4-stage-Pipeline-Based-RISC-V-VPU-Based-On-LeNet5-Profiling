...
extern int32_t in1_conv1[CONV1_IN_SIZE][CONV1_IN_SIZE + 3];
extern int32_t weights_conv1[CONV1_FEAT_SIZE][CONV1_KERN_SIZE * (CONV1_KERN_SIZE + 3)];
...
extern int32_t out_max1[MAXPOOL1_OUT_SIZE][MAXPOOL1_OUT_SIZE][MAXPOOL1_FEAT_SIZE+2];
...
extern int32_t weights_conv2[CONV2_KERN_SIZE * CONV2_KERN_SIZE][CONV2_FEAT_OUT_SIZE][CONV2_FEAT_IN_SIZE+2];
...
int main()
{
    ...
    for (int f = 0; f < CONV1_FEAT_SIZE; ++f) {
        for (int i1 = 0; i1 < CONV1_OUT_SIZE; ++i1) {
            for (int j1 = 0; j1 < CONV1_OUT_SIZE; ++j1) {
                ...
                int32_t *ptr_a = &in1_conv1[i1][j1];
                int32_t *ptr_b = &weights_conv1[f][0];
                int j2 = CONV1_KERN_SIZE * CONV1_KERN_SIZE;

                for (size_t vl; j2 > 0; j2 -= vl, ptr_a += vl, ptr_b += vl) {
                    vl = __riscv_vsetvl_e32m1(j2);
                    ...
                    vec_sum = __riscv_vmacc_vv_i32m1(vec_sum, vec_a, vec_b, vl);
                }
                
                // Perform reduction to compute the sum across the vector elements
                vec_sum = __riscv_vredsum_vs_i32m1_i32m1(vec_sum, vec_zero, vlmax);
    ...
    for (int i1 = 0; i1 < MAXPOOL1_OUT_SIZE; i1++) {
        for (int j1 = 0; j1 < MAXPOOL1_OUT_SIZE; j1++) {
            for (int f = 0; f < MAXPOOL1_FEAT_SIZE; f += vlmax) {
                ...
                size_t vl = __riscv_vsetvl_e32m1(MAXPOOL1_FEAT_SIZE - f);
                vint32m1_t max_val_vec = __riscv_vmv_v_x_i32m1(INT32_MIN, vl);
                
                for (int i2 = 0; i2 < 2; i2++) {
                    for (int j2 = 0; j2 < 2; j2++) {
                        vint32m1_t input_val_vec = __riscv_vle32_v_i32m1(&out_conv1[2 * i1 + i2][2 * j1 + j2][f], vl);
                        max_val_vec = __riscv_vmax_vv_i32m1(max_val_vec, input_val_vec, vl);
                    }
                }

                // Store the result
                __riscv_vse32_v_i32m1((int32_t *)&out_max1[i1][j1][f], max_val_vec, vl);
    ...
    for (int i1 = 0; i1 < CONV2_OUT_SIZE; ++i1)
    {
        for (int j1 = 0; j1 < CONV2_OUT_SIZE; ++j1)
        {
            for (int f = 0; f < CONV2_FEAT_OUT_SIZE; ++f)
            {
                ...
                {
                    int32_t *ptr_a = &out_max1[i1][j1][m];
                    int32_t *ptr_b = &weights_conv2[0][f][m];
    ...
    for (int i1 = 0; i1 < MAXPOOL2_OUT_SIZE; i1++) {
        for (int j1 = 0; j1 < MAXPOOL2_OUT_SIZE; j1++) {
            for (int f = 0; f < MAXPOOL2_FEAT_SIZE; f += vlmax) {
                ...
                size_t vl = __riscv_vsetvl_e32m1(MAXPOOL2_FEAT_SIZE - f);
                vint32m1_t max_val_vec = __riscv_vmv_v_x_i32m1(INT32_MIN, vl);
                
                for (int i2 = 0; i2 < 2; i2++) {
                    for (int j2 = 0; j2 < 2; j2++) {
                        vint32m1_t input_val_vec = __riscv_vle32_v_i32m1(&out_conv2[2 * i1 + i2][2 * j1 + j2][f], vl);
                        max_val_vec = __riscv_vmax_vv_i32m1(max_val_vec, input_val_vec, vl);
                    }
                }

                // Store the result
                __riscv_vse32_v_i32m1((int32_t *)&out_max2[i1][j1][f], max_val_vec, vl);
    ...
    for (int i1 = 0; i1 < FC1_OUT_SIZE; ++i1) {
        int32_t *ptr_a = &flatten[0];
        int32_t *ptr_b = &fc1[i1][0];
        int f = FC1_IN_SIZE;
    ...
    for (int i1 = 0; i1 < FC2_OUT_SIZE; ++i1) {
        int32_t *ptr_a = &out_fc1[0];
        int32_t *ptr_b = &fc2[i1][0];
        int f = FC2_IN_SIZE;
        vint32m1_t vec_s = __riscv_vmv_v_x_i32m1(0, vlmax);
        vint32m1_t vec_zero = __riscv_vmv_v_x_i32m1(0, vlmax);
        int32_t sum = 0;
    ...
    for (int i1 = 0; i1 < FC3_OUT_SIZE; ++i1) {
        int32_t *ptr_a = &out_fc2[0];
        int32_t *ptr_b = &fc3[i1][0];
        int f = FC3_IN_SIZE;
        vint32m1_t vec_s = __riscv_vmv_v_x_i32m1(0, vlmax);
        vint32m1_t vec_zero = __riscv_vmv_v_x_i32m1(0, vlmax);
        int32_t sum = 0;
