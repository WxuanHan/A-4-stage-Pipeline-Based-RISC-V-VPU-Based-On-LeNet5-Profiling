...
extern int8_t in1_conv1[CONV1_IN_SIZE][CONV1_IN_SIZE + 3];
extern int8_t weights_conv1[CONV1_FEAT_SIZE][CONV1_KERN_SIZE * (CONV1_KERN_SIZE + 3)];
extern int8_t out_conv1[CONV1_OUT_SIZE][CONV1_OUT_SIZE][CONV1_FEAT_SIZE];
...
extern int8_t out_max1[MAXPOOL1_OUT_SIZE][MAXPOOL1_OUT_SIZE][MAXPOOL1_FEAT_SIZE+2];
...
extern int8_t weights_conv2[CONV2_KERN_SIZE * CONV2_KERN_SIZE][CONV2_FEAT_OUT_SIZE][CONV2_FEAT_IN_SIZE+2];
extern int8_t out_conv2[CONV2_OUT_SIZE][CONV2_OUT_SIZE][CONV2_FEAT_OUT_SIZE];
...
extern int8_t out_max2[MAXPOOL2_OUT_SIZE][MAXPOOL2_OUT_SIZE][MAXPOOL2_FEAT_SIZE];

#define FC1_IN_SIZE (MAXPOOL2_OUT_SIZE * MAXPOOL2_OUT_SIZE * MAXPOOL2_FEAT_SIZE) // 256
#define FC1_OUT_SIZE 120

extern int8_t fc1[FC1_OUT_SIZE][FC1_IN_SIZE];
extern int8_t out_fc1[FC1_OUT_SIZE];

#define FC2_IN_SIZE FC1_OUT_SIZE // 120
#define FC2_OUT_SIZE 84

extern int8_t fc2[FC2_OUT_SIZE][FC2_IN_SIZE];
extern int8_t out_fc2[FC2_OUT_SIZE];

#define FC3_IN_SIZE FC2_OUT_SIZE // 84
#define FC3_OUT_SIZE 10

extern int8_t fc3[FC3_OUT_SIZE][FC3_IN_SIZE];
extern volatile int8_t out_fc3[FC3_OUT_SIZE];
...
int main()
{
    ...
    for (int f = 0; f < CONV1_FEAT_SIZE; ++f) {
        for (int i1 = 0; i1 < CONV1_OUT_SIZE; ++i1) {
            for (int j1 = 0; j1 < CONV1_OUT_SIZE; ++j1) {
                ...
                int8_t *ptr_a = &in1_conv1[i1][j1];
                int8_t *ptr_b = &weights_conv1[f][0];
    ...
    for (int i1 = 0; i1 < MAXPOOL1_OUT_SIZE; i1++) {
        for (int j1 = 0; j1 < MAXPOOL1_OUT_SIZE; j1++) {
            for (int f = 0; f < MAXPOOL1_FEAT_SIZE; f += vlmax) {
                ...
                size_t vl = __riscv_vsetvl_e8m1(MAXPOOL1_FEAT_SIZE - f);
                vint8m1_t max_val_vec = __riscv_vmv_v_x_i8m1(INT8_MIN, vl);
                
                for (int i2 = 0; i2 < 2; i2++) {
                    for (int j2 = 0; j2 < 2; j2++) {
                        vint8m1_t input_val_vec = __riscv_vle8_v_i8m1(&out_conv1[2 * i1 + i2][2 * j1 + j2][f], vl);
                        max_val_vec = __riscv_vmax_vv_i8m1(max_val_vec, input_val_vec, vl);
                    }
                }

                // Store the result
                __riscv_vse8_v_i8m1((int8_t *)&out_max1[i1][j1][f], max_val_vec, vl);
    ...
    for (int i1 = 0; i1 < CONV2_OUT_SIZE; ++i1)
    {
        for (int j1 = 0; j1 < CONV2_OUT_SIZE; ++j1)
        {
            for (int f = 0; f < CONV2_FEAT_OUT_SIZE; ++f)
            {
                ...
                {
                    int8_t *ptr_a = &out_max1[i1][j1][m];
                    int8_t *ptr_b = &weights_conv2[0][f][m];
    ...
    for (int i1 = 0; i1 < MAXPOOL2_OUT_SIZE; i1++) {
        for (int j1 = 0; j1 < MAXPOOL2_OUT_SIZE; j1++) {
            for (int f = 0; f < MAXPOOL2_FEAT_SIZE; f += vlmax) {
                ...
                size_t vl = __riscv_vsetvl_e8m1(MAXPOOL2_FEAT_SIZE - f);
                vint8m1_t max_val_vec = __riscv_vmv_v_x_i8m1(INT8_MIN, vl);
                
                for (int i2 = 0; i2 < 2; i2++) {
                    for (int j2 = 0; j2 < 2; j2++) {
                        vint8m1_t input_val_vec = __riscv_vle8_v_i8m1(&out_conv2[2 * i1 + i2][2 * j1 + j2][f], vl);
                        max_val_vec = __riscv_vmax_vv_i8m1(max_val_vec, input_val_vec, vl);
                    }
                }

                // Store the result
                __riscv_vse8_v_i8m1((int8_t *)&out_max2[i1][j1][f], max_val_vec, vl);
    ...
    // flatten
    int8_t *flatten = (int8_t *)&out_max2;

    // fully connected layer 1
    for (int i1 = 0; i1 < FC1_OUT_SIZE; ++i1) {
        int8_t *ptr_a = &flatten[0];
        int8_t *ptr_b = &fc1[i1][0];
        int f = FC1_IN_SIZE;
    ...
    for (int i1 = 0; i1 < FC2_OUT_SIZE; ++i1) {
        int8_t *ptr_a = &out_fc1[0];
        int8_t *ptr_b = &fc2[i1][0];
        int f = FC2_IN_SIZE;
        vint8m1_t vec_s = __riscv_vmv_v_x_i8m1(0, vlmax);
        vint8m1_t vec_zero = __riscv_vmv_v_x_i8m1(0, vlmax);
        int32_t sum = 0;
    ...
    for (int i1 = 0; i1 < FC3_OUT_SIZE; ++i1) {
        int8_t *ptr_a = &out_fc2[0];
        int8_t *ptr_b = &fc3[i1][0];
        int f = FC3_IN_SIZE;
        vint8m1_t vec_s = __riscv_vmv_v_x_i8m1(0, vlmax);
        vint8m1_t vec_zero = __riscv_vmv_v_x_i8m1(0, vlmax);
        int32_t sum = 0;
